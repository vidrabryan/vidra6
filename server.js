/* Url: https://repl.it/@jimboy3100/agario-botsNEW
 Websocket: wss://agario-botsNEW--jimboy3100.repl.co
...
Example https://repl.it/@jimboy3100/agario-bots3
Websocket: wss://agario-bots3--jimboy3100.repl.co
...
logger.warn(`[SERVER] REMEMBER: BEFORE CHANGING SERVER, First stop BOTS and wait 30 seconds, then change server, run this server and connect to bots`)
...
class Bot {
    ...
    updateOffset(reader) {
        ...
        if (~~(right - left) === dataBot.mapSize && ~~(bottom - top) === dataBot.mapSize) {
        ...
        if (user.isAlive) {
            if (this.followMouse && !stoppingBots && !bots.ai) this.sendPosition(user.mouseX + this.offsetX, user.mouseY + this.offsetY)
                ...
                if (closestBiggerPlayer.entity && closestBiggerPlayer.distance < 420) {
                    ...
                    this.sendPosition(dataBot.mapSize * Math.cos(angle), dataBot.mapSize * Math.sin(angle))
                } 
        ...
        switch (reader.readUint8()) {
            case 0:
                if (!user.startedBots) {
					...
					//bots.name2 = decodeURIComponent(escape(bots.name))
					console.log(bots.name , bots.name2)
